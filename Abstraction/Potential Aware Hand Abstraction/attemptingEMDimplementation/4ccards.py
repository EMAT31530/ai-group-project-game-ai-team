# taken from https://github.com/ChengTsang/Potential-Aware-Abstraction-In-Texas-Hold-em-Poker


"""Script that drives the cluster algorithm for 2 hands and 4 public
"""
import numpy
    ...
    def solve(self, dataPoints, k):
        ...
        while (notDone):
          ...
          # See if the kmean algorithm has converged
            ...
            clusters = self.configure_clusters(centroids, dataPoints)
        #(centroids)
        with open("centroids_2.csv",'w') as file:
            for i in centroids:
                file.write(str(i.tolist()[0])+','+str(i.tolist()[1])+','+str(i.tolist()[2])+','+str(i.tolist()[3])+','+str(i.tolist()[4])+'\n')
...
def main():
    ...
    dataPoints = []
    with open("data_2.csv") as file:
        for line in file:
            string_line = line.strip().split(",")[:5]
            #print(type(string_line))
            dataPoint = list(map(float, string_line))
            dataPoints.append(dataPoint)
    print(len(dataPoints))
    kmean = Clustering()
    kmean.solve(dataPoints, 10)
