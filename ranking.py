...
def straight_flush(card_suits, consec_cards, hand): #all the following functions check to see if you have the following hand
    ...
    if consec_cards == False or list(card_suits.values())[0] < 5:
        ...
        for i in hand.cards:
            if i.suit == suit:
                suit_hand.addCard(i)
        return 9, consecutive(suit_hand) + 4 #return top card for easier comparison
...
def four_of_a_kind(card_nums):
    if list(card_nums.values())[0] != 4:
        ...
        if len(list(card_nums.keys())) == 1:
            return 8, list(card_nums.keys())[0], 0 #if the hand is only four cards (not technically possible in a poker game but anyway) then by default return the four of a kind and zero
        else:
